...
class Data_Reg_Binary(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return img, gt_mask_bin, gt_distelf.preprocess(img, gt_mask_bin)
...
class Data_Binary(Dataset):
    ...
    def transform(self,sample):
        ...
        image = torch.from_numpy(image.astype(np.float32)).unsqueeze(0)
        label = torch.from_numpy(label.astype(np.float32))  #can be changed
        sample = {'image': image, 'label': label.long()}
        return sample
    ...
    def __getitem__(self, idx):
        ...
        if "test" in label_path:
            label_path=label_path.replace("test", "golds")  #changed herre
...
class Data_Reg_Fourier1(Dataset):
    ...
    def preprocess(self, img, mask, fmap):
    ...
    def __getitem__(self, index):
        ...
        # Preprocess
        img, gt_mask_bin, gt_fmap1 = self.preprocess(
...
class Data_Reg_Fourier1_2(Dataset):
    ...
    def preprocess(self, img, mask, fmap1, fmap2):
    ...
    def __getitem__(self, index):
        ...
        # Preprocess
        img, gt_mask_bin, gt_fmap1, gt_fmap2 = self.preprocess(
